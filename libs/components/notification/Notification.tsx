...
// ====== Config ======
const PAGE_LIMIT = 5;
const SOFT_DELETE_ONLY = true; // set to false if you later want real server-side delete
...
// Persisted storage key for hidden (soft-deleted) notifications
...
const NotificationBell: React.FC = () => {
  ...
  // Local pagination + list state
  const [page, setPage] = useState(1);
  const [items, setItems] = useState<any[]>([]);
  const [total, setTotal] = useState(0);

  // Soft-delete IDs (persisted)
  ...
  // ====== GraphQL ======
  const { loading, fetchMore, refetch } = useQuery<GetNotificationsData, GetNotificationsVars>(
    GET_NOTIFICATIONS,
    {
  ...
  const [markAll, { loading: markingAll }] = useMutation<MarkAllNotificationsReadData>(
    MARK_ALL_NOTIFICATIONS_READ
  );

  // Live updates: refetch first page then reset
  useNotificationWS({
    userId: user?._id,
    client,
    onPing: async () => {
  ...
  // Try to keep the viewport filled after items are hidden
  useEffect(() => {
  ...
  const handleMarkAll = async () => {
    ...
    // Ensure server/client totals sync
    const res = await refetch({ input: { page: 1, limit: PAGE_LIMIT } });
    setPage(1);
    setItems(res.data?.getNotifications?.list ?? []);
    setTotal(res.data?.getNotifications?.total ?? 0);
    ...
    <>
      ...
      >
        ...
        <Box
          ref={listBoxRef}
          onScroll={onScroll}
          sx={{ overflowY: 'auto', maxHeight: 420, py: 0.5 }}
        >
                  ...
                  return (
                    <Box
                      ...
                      sx={{
                        ...
                        '&:hover': { transform: 'translateY(-1px)', background: '#fffdfa', border: '1px solid rgba(212,180,131,0.65)' },
                      ...
                      <ListItem
                        ...
                        onClick={async (e: any) => {
                          ...
                          if (isUnread && !markingOne) {
                            await handleMarkOne(n._id); // mark read only; do not navigate or close
                          }
                        }}
                        secondaryAction={
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            ...
                            <Tooltip title="Remove notification">
                              <IconButton
                                ...
                                onClick={(e: any) => {
                                  e.stopPropagation();
                                  // Soft delete for now; if you add hard delete later, call a mutation here
                                  SOFT_DELETE_ONLY ? handleSoftRemove(n._id) : handleSoftRemove(n._id);
                                }}
                                sx={{ color: '#6b5a45', '&:hover': { color: '#2e2424', bgcolor: 'rgba(212,180,131,0.15)' } }}
                              >
                      ...
                      >
                        <ListItemAvatar sx={{ mr: 1.25 }}>
                          <Avatar
                            ...
                            imgProps={{
                              ...
                              onError: (e: any) => { e.currentTarget.src = '/img/profile/defaultUser3.svg'; },
                            }}
                          />
                        </ListItemAvatar>
                        ...
                        <ListItemText
                          ...
                          secondary={
                            secondary ? (
                              <Typography
                                sx={{ color: '#5d5247', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}
                                variant="body2"
                              >
                                {secondary}
                              </Typography>
