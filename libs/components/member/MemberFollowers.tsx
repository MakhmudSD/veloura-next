...
const MemberFollowers = (props: MemberFollowsProps) => {
	...
	const {
		...
		onCompleted: (data: T) => {
			console.log('Followers Data:', data); // Log the entire data object
	...
	/** LIFECYCLES **/
	useEffect(() => {
		// Correctly set followingId for fetching followers
		if (router.query.memberId) {
			...
			setFollowInquiry({
				...followInquiry,
				search: { followingId: user?._id as string },
			});
		}
	}, [router, user]);

	useEffect(() => {
		// Refetch only when followInquiry.search.followingId is valid
